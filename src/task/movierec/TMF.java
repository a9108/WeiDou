...
public class TMF extends RankingTask {
	...
	public void train() {
		...
		model = new MatrixFactorization(data.getSizeDouban(),
				data.getSizeMovie(), 10, 50, 1e-2, 1e-4, 1e-1,
				MatrixFactorization.SIGMOID);

//		showSimAnalyze_Jacc();

		if (false) for (int i = 0; i < data.getSizeDouban(); i++) {
			ArrayList<Double> topics = theta.get(data.getLinks().get(i));
			for (int j = 0; j < topics.size(); j++)
				model.addUserFeature(i, j, topics.get(j));
		}
